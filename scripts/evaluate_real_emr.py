#!/usr/bin/env python3
"""
Evaluation Script for Real EMR-Only Pneumonia Classification Model
Evaluates trained model on test set and generates comprehensive metrics and visualizations
"""
...
from config import CHECKPOINTS_DIR, FIG_DIR, LOGS_DIR
from utils_io import ensure_dir, load_torch, load_numpy, load_json, save_json

class RealEMRDataset(torch.utils.data.Dataset):
    """Real EMR dataset for evaluation"""
...
class ModelEvaluator:
    ...
    def plot_confusion_matrix(self, predictions, targets, save_path):
        ...
        plt.title('Real EMR Model - Confusion Matrix', fontsize=14, fontweight='bold')
    ...
    def plot_roc_curve(self, targets, probabilities, save_path):
        ...
        plt.title('Real EMR Model - ROC Curve', fontsize=14, fontweight='bold')
    ...
    def plot_precision_recall_curve(self, targets, probabilities, save_path):
        ...
        plt.title('Real EMR Model - Precision-Recall Curve', fontsize=14, fontweight='bold')
    ...
    def plot_metrics_bar_chart(self, metrics, save_path):
        ...
        plt.title('Real EMR Model - Performance Metrics', fontsize=14, fontweight='bold')
    ...
    def create_combined_results_summary(self, predictions, targets, probabilities, metrics, save_path):
        ...
        fig.suptitle('Real EMR-Only Pneumonia Classification - Results Summary', 
...
def evaluate_model(model_path, test_loader, device, output_dir):
    ...
    print("🚀 Starting Real EMR Model Evaluation...")
    print("="*60)
    ...
    # Individual plots
    evaluator.plot_confusion_matrix(predictions, targets, output_dir / "real_emr_confusion_matrix.png")
    print(f"   ✅ Confusion matrix saved to: {output_dir / 'real_emr_confusion_matrix.png'}")
    
    evaluator.plot_roc_curve(targets, probabilities, output_dir / "real_emr_roc_curve.png")
    print(f"   ✅ ROC curve saved to: {output_dir / 'real_emr_roc_curve.png'}")
    
    evaluator.plot_precision_recall_curve(targets, probabilities, output_dir / "real_emr_pr_curve.png")
    print(f"   ✅ Precision-Recall curve saved to: {output_dir / 'real_emr_pr_curve.png'}")
    
    evaluator.plot_metrics_bar_chart(metrics, output_dir / "real_emr_metrics_bar.png")
    print(f"   ✅ Metrics bar chart saved to: {output_dir / 'real_emr_metrics_bar.png'}")
    
    # Combined summary
    evaluator.create_combined_results_summary(predictions, targets, probabilities, metrics, 
                                            output_dir / "real_emr_results_summary.png")
    print(f"   ✅ Combined results summary saved to: {output_dir / 'real_emr_results_summary.png'}")
    
    # Classification report
    evaluator.generate_classification_report(predictions, targets, output_dir / "real_emr_classification_report.json")
    print(f"   ✅ Classification report saved to: {output_dir / 'real_emr_classification_report.json'}")
    
    # Save metrics
    save_json(metrics, output_dir / "real_emr_test_metrics.json")
    print(f"✅ Saved JSON: {output_dir / 'real_emr_test_metrics.json'}")
    
    # Append to training log
    try:
        train_log_path = LOGS_DIR / "real_emr_train_log.csv"
    ...
    print("="*60)
    print("✅ Real EMR evaluation completed successfully!")
...
def main():
    ...
    parser = argparse.ArgumentParser(description="Evaluate Real EMR-Only Pneumonia Classification Model")
    parser.add_argument('--model_path', type=str, 
                       default=str(CHECKPOINTS_DIR / "real_emr_best.pt"),
    ...
    if not model_path.exists():
        ...
        print("   python scripts/train_real_emr.py --epochs 10 --batch_size 32 --lr 1e-4")
        sys.exit(1)
    
    # Check if test data exists
    real_emr_proc_dir = Path("data/processed/real_emr")
    required_files = [
        real_emr_proc_dir / "X_real_emr_test.npy",
        real_emr_proc_dir / "y_real_emr_test.npy"
    ]
    
    missing_files = [f for f in required_files if not f.exists()]
    if missing_files:
        print("❌ Missing test data files:")
    ...
    # Load test data
    print("\n📊 Loading test data...")
    X_test = load_numpy(real_emr_proc_dir / "X_real_emr_test.npy")
    y_test = load_numpy(real_emr_proc_dir / "y_real_emr_test.npy")
    
    print(f"   Test samples: {len(X_test)}")
    
    # Create test dataset and dataloader
    test_dataset = RealEMRDataset(X_test, y_test)
