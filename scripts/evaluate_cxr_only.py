#!/usr/bin/env python3
"""
Evaluation Script for CXR-Only Pneumonia Classification Model
Evaluates trained CXR-only model and generates comprehensive metrics and visualizations
"""
...
from data_module_cxr_only import make_cxr_only_dataloaders
from config import PROC_DIR, RSNA_ROOT, BATCH_SIZE, NUM_WORKERS, FIG_DIR, LOGS_DIR, LABEL_MAP
from utils_io import load_torch, ensure_dir, save_json

class CXROnlyModelEvaluator:
    """Comprehensive CXR-only model evaluation with metrics and visualizations"""
    ...
    def evaluate_dataloader(self, dataloader, split_name="test"):
        ...
        with torch.no_grad():
            ...
            for batch in progress_bar:
                ...
                # Forward pass
                logits, probabilities = self.model(images)
    ...
    def plot_confusion_matrix(self, predictions, targets, save_path):
        ...
        plt.title('CXR-Only Model - Confusion Matrix', fontsize=16, fontweight='bold')
    ...
    def plot_roc_curve(self, targets, probabilities, save_path):
        ...
        plt.title('CXR-Only Model - ROC Curve', fontsize=16, fontweight='bold')
    ...
    def plot_precision_recall_curve(self, targets, probabilities, save_path):
        ...
        plt.title('CXR-Only Model - Precision-Recall Curve', fontsize=16, fontweight='bold')
    ...
    def plot_metrics_bar_chart(self, metrics, save_path):
        ...
        plt.title('CXR-Only Model - Performance Metrics', fontsize=16, fontweight='bold')
    ...
    def create_combined_results_summary(self, predictions, targets, probabilities, metrics, save_path):
        ...
        # Create summary text
        summary_text = f"""
CXR-Only Model Performance Summary
...
• False Negatives: {metrics['false_negatives']:,}
        ...
        # Overall title
        fig.suptitle('CXR-Only Pneumonia Diagnosis Model - Final Evaluation Results', 
...
def evaluate_cxr_only_model(model_path, test_loader, device, output_dir):
    """Main evaluation function"""
    print("🚀 Starting CXR-Only Model Evaluation...")
    print("="*60)
    ...
    # Create evaluator
    evaluator = CXROnlyModelEvaluator(model, device)
    ...
    print("="*60)
    print("✅ CXR-only evaluation completed successfully!")
...
def main():
    """Main evaluation function"""
    parser = argparse.ArgumentParser(description="Evaluate CXR-Only Pneumonia Classification Model")
    parser.add_argument('--model_path', type=str, default='outputs/checkpoints/cxr_only_best.pt', help='Path to trained model')
    ...
    # Evaluate model
    metrics = evaluate_cxr_only_model(args.model_path, test_loader, device, output_dir)
